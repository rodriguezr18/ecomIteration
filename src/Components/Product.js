    ...
    return (
        ...
        <h2>Products</h2>
            <section className="grid">
                {data.map(prod => (
                    <ProductCards title={prod.productName  +" | "+  prod.artistName} imageSrc={prod.imgSource} price={ "$" + prod.price} description={prod.description}  imgAlt={`${prod.productName} ${prod.artistName} `} key={prod.id}/>
                    ))}
            </section>
        </>
    )
